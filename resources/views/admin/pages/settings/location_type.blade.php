...
@section('title', 'Location Type Management')
            ...
            <div class="col-xl-12">
                ...
                <div class="row">
                    <ul class="breadcrumb">
                        ...
                        <li class="breadcrumb-item text-uppercase active">Location Type Management</li>
                    </ul>
                    <div class="d-flex justify-content-between align-items-center">
                        <h1 class="page-header">
                            Location Type Management <small>create update delete all in here</small>
                        </h1>
                        ...
                        <div class="col-xl-12">
                            <div class="card border-success bg-success bg-opacity-25 mb-4">
                                <div class="card-header border-success fw-bold small text-body">
                                    <div class="row">
                                        <div class="col">
                                            <h4 class="card-title m-0">Location Type Management</h4>
                                        </div>
                                        <div class="col-auto">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title">Create new location type info</h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('settings.location_type.store') }}" method="POST" id="create_update_form"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <input type="text" class="form-control" placeholder="On-site"
                        ...
                        <div class="form-group mb-3">
                            ...
                            <textarea class="form-control" name="details" id="details" placeholder="On-site opportunity in Bangladesh office"></textarea>
                        </div>
...
@section('page_script')
    <script>
        $('#create_update_modal').on('hidden.bs.modal', function() {
            ...
            $('#create_update_form').attr('action', '{{ route('settings.location_type.store') }}');
            $('#create_update_form_method').val('POST');
            $('#location_type_id').val('');
        });

        $(document).on('click', '.btn-edit', function() {
            let location_type_id = $(this).data('location_type_id');

            $.ajax({
                url: "{{ route('settings.location_type.edit', ':id') }}".replace(':id', location_type_id),
                type: 'GET',
                success: function(response) {
                    // Fill the form fields
                    $('#location_type_id').val(response.id);
                    $('#title').val(response.value);
                    $('#details').val(response.details);
                    $('#status').val(response.status);

                    const form = document.getElementById('create_update_form');
                    form.action = "{{ route('settings.location_type.update') }}";
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.querySelectorAll('.btn-delete').forEach(button => {
                button.addEventListener('click', function() {
                    const location_type_id = this.getAttribute('data-location_type_id');
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href =
                                "{{ route('settings.location_type.delete', ':id') }}"
                                .replace(':id', location_type_id);
