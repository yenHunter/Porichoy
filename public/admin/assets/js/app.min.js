...
var slideUp = function(elm, duration = app.animation.speed) {
	if (!elm.classList.contains('transitioning')) {
		...
		elm.style.transition = `height ${duration}ms ease, padding ${duration}ms ease, opacity ${duration}ms ease`;
		elm.style.willChange = 'height, padding, opacity';
		...
		window.setTimeout(() => {
			...
			elm.style.removeProperty('will-change');
...
var slideDown = function(elm, duration = app.animation.speed) {
	if (!elm.classList.contains('transitioning')) {
		...
		requestAnimationFrame(() => {
			elm.style.height = height + 'px';
			elm.style.paddingTop = paddingTop;
			elm.style.paddingBottom = paddingBottom;
			elm.style.opacity = '1';
		});
